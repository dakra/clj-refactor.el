...
- **(Breaking)** Drop the `yasnippet` dependency, along with the commands that relied on its snippet expansion: `cljr-add-require-to-ns` (`ar`), `cljr-add-use-to-ns` (`au`), `cljr-add-import-to-ns` (`ai`) and `cljr-require-macro` (`rm`). Edit the `ns` form directly, or use `cljr-slash`, `cljr-clean-ns` and `cljr-add-missing-libspec`. `cljr-create-fn-from-example` is unaffected; it now inserts a plain function stub instead of a snippet.
